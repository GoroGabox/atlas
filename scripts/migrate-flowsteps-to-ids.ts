...
function parseEndpoints(json: string | null): Array<{ method?: string; path?: string }> {
  if (!json) return [];
  try { return JSON.parse(json) as Array<{ method?: string; path?: string }>; }
  catch { return []; }
}
...
async function main() {
  ...
  const [screens, components, services, endpoints, steps] = await Promise.all([
  ...
  const screenByName   = new Map(screens.map((s) => [normalize(s.name), s.id]));
  const compByName     = new Map(components.map((c) => [normalize(c.name), c.id]));
  const serviceByName  = new Map(services.map((s) => [normalize(s.name), s.id]));
  const endpointByKey  = new Map(endpoints.map((e) => [`${e.method.toUpperCase()}:${normalize(e.path)}`, e.id]));
  ...
  for (const step of steps) {
    ...
    if (compNames.length > 0) {
      ...
      for (const name of compNames) {
        ...
        if (id) { resolvedIds.push(id); report.compsResolved++; }
        else    { report.compsLost++; report.unresolvableItems.push(`step[${step.id}].components = "${name}"`); }
      }
    ...
    if (respCompNames.length > 0) {
      ...
      for (const name of respCompNames) {
        const id = compByName.get(normalize(name));
        if (id) { resolvedIds.push(id); report.compsResolved++; }
        else    { report.compsLost++; report.unresolvableItems.push(`step[${step.id}].responseComponents = "${name}"`); }
      }
    ...
    if (svcNames.length > 0) {
      ...
      for (const name of svcNames) {
        ...
        if (id) { resolvedIds.push(id); report.svcsResolved++; }
        else    { report.svcsLost++; report.unresolvableItems.push(`step[${step.id}].services = "${name}"`); }
      }
    ...
    if (epObjs.length > 0) {
      ...
      for (const ep of epObjs) {
        ...
        if (id) { resolvedIds.push(id); report.epsResolved++; }
        else    { report.epsLost++; report.unresolvableItems.push(`step[${step.id}].endpoints = "${ep.method} ${ep.path}"`); }
      }
    ...
    if (changed) {
      ...
      if (!DRY_RUN) {
        await prisma.flowStep.update({ where: { id: step.id }, data: update });
      }
    }
  }
